...
@pytest.mark.importance("critical")
@pytest.mark.ticket(jira=["RHEL-115442"])
@pytest.mark.topology(KnownTopology.BareLDAP)
@pytest.mark.xfail(reason="SSSD bug: sudoUser group GID notation (%#GID) is not supported")
def test_basic__sudouser_with_group_gid(client: Client, provider: LDAP):
    """
    ...
    provider.sudorule("rule1").add(user="%#20001", host="ALL", command="ALL")
...
@pytest.mark.topology(KnownTopology.BareLDAP)
@pytest.mark.xfail(reason="SSSD bug: sudoUser group GID notation (%#GID) is not supported")
def test_basic__sudouser_with_multiple_group_gids(client: Client, provider: LDAP):
    """
    ...
    provider.sudorule("defaults").add(option="!authenticate")
    gid_users: Any = ["%#30001", "%#30002"]
    provider.sudorule("multi-gid").add(user=gid_users, host="ALL", command="ALL")
...
@pytest.mark.topology(KnownTopology.BareLDAP)
@pytest.mark.xfail(reason="SSSD bug: sudoUser group GID notation (%#GID) is not supported")
def test_basic__sudouser_group_gid_with_group_name_mixed(client: Client, provider: LDAP):
    """
    ...
    provider.sudorule("defaults").add(option="!authenticate")
    mixed_users: Any = ["%#40001", "%devgroup"]
    provider.sudorule("mixed").add(user=mixed_users, host="ALL", command="ALL")
